...
Future<void> main() async {
  ...
  // Configure the audio session for background playback so focus music keeps
  // playing through a locked screen / backgrounded focus session (ADR-0005).
  try {
    final session = await AudioSession.instance;
    await session.configure(const AudioSessionConfiguration.music());
  } catch (_) {
    // Audio session unavailable in this environment; non-fatal.
  }

  final okrStorage = OkrStorage();
  final pomodoroStorage = PomodoroStorage();
  final provider =
      StorageProvider(okrStorage: okrStorage, pomodoroStorage: pomodoroStorage);
  provider.focusMusic = FocusMusicController(JustAudioFocusMusicPlayer());
